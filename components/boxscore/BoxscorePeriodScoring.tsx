    ...
    >
      ...
      <div
        className={classnames(
          'flex-1 text-center',
          teamColorIsDark && 'text-grey100 dark:!text-white',
        )}
        style={{
...
const PeriodScoringColumn = ({
  ...
  return (
    <>
      <div className="w-full border-b border-b-grey500 bg-grey300 p-2.5 font-mont text-sm text-grey700 dark:border-b-globalBorderGrey dark:bg-globalBackgroundGrey dark:text-white">
        {title}
      </div>
      {!data ? (
        ...
        data.map((goal, i) => {
          return (
            <div
              key={i}
              className={classnames(
                'flex w-full items-center border-b border-b-grey500 py-6 px-2.5 dark:border-b-globalBorderGrey',
              )}
            >
              <TeamLogo
                league={league}
              ...
              <div className="flex w-full flex-col overflow-hidden text-ellipsis">
                <Link
                  ...
                  className="w-full overflow-hidden text-ellipsis whitespace-nowrap text-xl font-semibold hover:text-blue600 dark:hover:text-hyperlink"
                >
                  {goal.scorer.name}
                </Link>
                <div className="text-xs text-grey700 dark:text-white">
                  {!goal.primaryAssist
                    ? 'Unassisted'
...
export const BoxscorePeriodScoring = ({
  ...
  return (
    <>
      <div className="mt-1 ml-4 text-sm font-medium text-grey700 dark:text-white">Scoring</div>
