...
export default async (
  ...
  const shgLeaders = await query(SQL`
    SELECT s.PlayerID, s.LeagueID, s.SeasonID, s.TeamID, p.\`Last Name\` AS Name, s.SHG as ShorthandedGoals
    ...
    ORDER BY ShorthandedGoals `).append(desc ? `DESC` : `ASC`).append(`
    LIMIT ${limit}
    `));
