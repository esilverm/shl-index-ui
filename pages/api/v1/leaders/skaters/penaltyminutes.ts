...
export default async (
  ...
  const penaltyLeaders = await query(SQL`
    SELECT s.PlayerID, s.LeagueID, s.SeasonID, s.TeamID, p.\`Last Name\` AS Name, s.PIM as PenaltyMinutes
    ...
    ORDER BY PenaltyMinutes `).append(desc ? `DESC` : `ASC`).append(`
    LIMIT ${limit}
    `));
