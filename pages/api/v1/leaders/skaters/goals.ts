...
export default async (
  ...
  const goalLeaders = await query(SQL`
    SELECT s.PlayerID, s.LeagueID, s.SeasonID, s.TeamID, p.\`Last Name\` AS Name, s.G as Goals
    ...
    ORDER BY Goals `).append(desc ? `DESC` : `ASC`).append(`
    LIMIT ${limit}
    `));
