...
export default async (
  ...
  const spctLeaders = await query(SQL`
    SELECT s.PlayerID, s.LeagueID, s.SeasonID, s.TeamID, p.\`Last Name\` AS Name, (s.G / NULLIF(s.SOG, 0)) as ShotPct
    ...
    ORDER BY ShotPct `).append(desc ? `DESC` : `ASC`).append(`
    LIMIT ${limit}
    `));
