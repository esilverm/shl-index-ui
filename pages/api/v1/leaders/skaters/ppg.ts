...
export default async (
  ...
  const ppgLeaders = await query(SQL`
    SELECT s.PlayerID, s.LeagueID, s.SeasonID, s.TeamID, p.\`Last Name\` AS Name, s.PPG as PowerPlayGoals
    ...
    ORDER BY PowerPlayGoals `).append(desc ? `DESC` : `ASC`).append(`
    LIMIT ${limit}
    `));
