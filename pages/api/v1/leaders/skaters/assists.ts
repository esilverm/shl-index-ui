...
export default async (
  ...
  const assistLeaders = await query(SQL`
    SELECT s.PlayerID, s.LeagueID, s.SeasonID, s.TeamID, p.\`Last Name\` AS Name, s.A as Assists
    ...
    ORDER BY Assists `).append(desc ? `DESC` : `ASC`).append(`
    LIMIT ${limit}
    `));
