...
export default async (
  ...
  const pmLeaders = await query(SQL`
    SELECT s.PlayerID, s.LeagueID, s.SeasonID, s.TeamID, p.\`Last Name\` AS Name, s.plusMinus
    ...
    ORDER BY plusMinus `).append(desc ? `DESC` : `ASC`).append(`
    LIMIT ${limit}
    `));
