...
export default async (
  ...
  const gaLeaders = await query(SQL`
    SELECT s.PlayerID, s.LeagueID, s.SeasonID, s.TeamID, p.\`Last Name\` AS Name, s.GoalsAgainst
    ...
    ORDER BY s.GoalsAgainst `).append(desc ? `DESC` : `ASC`).append(`
    LIMIT ${limit}
    `));
