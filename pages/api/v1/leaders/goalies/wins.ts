...
export default async (
  ...
  const winLeaders = await query(SQL`
    SELECT s.PlayerID, s.LeagueID, s.SeasonID, s.TeamID, p.\`Last Name\` AS Name, s.Wins
    ...
    ORDER BY s.Wins `).append(desc ? `DESC` : `ASC`).append(`
    LIMIT ${limit}
    `));
