...
export default async (
  ...
  const otLeaders = await query(SQL`
    SELECT s.PlayerID, s.LeagueID, s.SeasonID, s.TeamID, p.\`Last Name\` AS Name, s.OT
    ...
    ORDER BY s.OT `).append(desc ? `DESC` : `ASC`).append(`
    LIMIT ${limit}
    `));
